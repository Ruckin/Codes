...
y_start = endEffector(x,a1,a2);
y_goal  = [0.25; 0.15];

params = struct( ...
    'a1',a1,'a2',a2,'ac1',ac1,'ac2',ac2,...
    'm1',m1,'m2',m2,'I1',I1,'I2',I2,'g',g,...
    'Ts',Ts,'u_min',u_min,'u_max',u_max,...
    'alpha0',0.3,'alpha_min',0.02,'max_wp',5);

waypoints = generateAdmissibleWaypoints( ...
    y_start, y_goal, x, limits, params);

durations = 60*ones(1,size(waypoints,2));
...
function waypoints = generateAdmissibleWaypoints( ...
    y_start, y_goal, x0, limits, params)
...
for i = 1:params.max_wp

    y_candidate = y_curr + alpha*(y_goal - y_curr);

    [okKin, q] = checkKinematicReachability( ...
        y_candidate, params.a1, params.a2, limits);

    okDyn = false;
    if okKin
        okDyn = checkDynamicReachability( ...
            q, x0, params.Ts, limits, ...
            params.a1, params.a2, params.ac1, params.ac2, ...
            params.m1, params.m2, params.I1, params.I2, ...
            params.g, params.u_min, params.u_max);
    end

    if okKin && okDyn
        waypoints = [waypoints y_candidate];
        y_curr = y_candidate;
        alpha  = params.alpha0;  % reset step
    else
        alpha = alpha / 2;
        if alpha < params.alpha_min
            warning('Cannot find further admissible waypoint.');
            break;
        end
    end

    if norm(y_curr - y_goal) < 1e-2
        break;
    end
end
end
